...
class App(ctk.CTk): ####### ↓↓↓ Main Window or Root ↓↓↓ #######
    def __init__(self):
        super().__init__()
        self.title("CERTIGEN")
        self.iconbitmap('images/icon.ico') # CERTIGEN window icon
        self.geometry(f"{self.winfo_screenwidth()}x{self.winfo_screenheight()}") # getting user's screen size
        self.state('zoomed')
        self.title = ctk.CTkLabel(self, text='CERTIGEN', font=('fonts/Montserrat-Bold.ttf', 50)) # Setting title
        self.tagline = ctk.CTkLabel(self, text='Certificate automation and generation software', font=normalFont) # Setting tagline
